...
http://api.example.com/users?limit=2&total=true
...
`total` must be defined as a query parameter.
...
Assuming `X-Total-Count` is defined as a response header. Header names are case-insensitive.
